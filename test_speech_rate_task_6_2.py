#!/usr/bin/env python3
"""
Test Script for Sub-task 6.2: Test speech rate controls with one voice per language

Tests ONE representative voice from each of 5 languages with 2 different rate settings:
- Slow (0.75x / "-25%")
- Fast (1.5x / "+50%")
...
TEST_VOICES = {
    ...
    "Spanish": {
        ...
        "text": "Esta es una prueba de control de velocidad."
    },
    "French": {
        "voice": "fr-FR-DeniseNeural",
        "text": "Ceci est un test de contrôle de la vitesse."
    },
    "German": {
        "voice": "de-DE-AmalaNeural",
        "text": "Dies ist ein Test der Sprachgeschwindigkeitssteuerung."
    },
    "Portuguese": {
        "voice": "pt-BR-AntonioNeural",
        "text": "Este é um teste de controle de velocidade."
    }
}

# Rate settings to test (2 rates as specified in task)
RATE_TESTS = {
    "Slow (0.75x)": "-25%",
    "Fast (1.5x)": "+50%"
}
...
def run_tests() -> Tuple[List[Dict], Dict]:
    """
    Run all speech rate control tests.

    Returns:
        Tuple of (all_results, summary_stats)
    """
    print("=" * 80)
    print("SUB-TASK 6.2: TEST SPEECH RATE CONTROLS")
    print("=" * 80)
    print(f"\nAPI Endpoint: {GENERATE_ENDPOINT}")
    print(f"\nTesting {len(TEST_VOICES)} languages × {len(RATE_TESTS)} rates each")
    ...
    for language, config in TEST_VOICES.items():
        ...
        for rate_label, rate_value in RATE_TESTS.items():
            print(f"  Testing {rate_label} (rate={rate_value})...", end=" ")
...
def print_summary(results: List[Dict], summary: Dict):
    ...
    print("\n" + "=" * 80)
    print("TASK 6.2 COMPLETION STATUS")
    print("=" * 80)

    if summary["failed"] == 0 and summary["successful"] >= 10:
        print("\n✓ TASK COMPLETE - All requirements met:")
        print(f"  • {summary['successful']} TTS generations succeeded (5 languages × 2 rates)")
        print("  • All API calls returned 200 with valid audio_url")
        print("  • Rate parameter accepted without errors for all languages")
        print("  • Speech rate controls verified working across all languages")
        return 0
    else:
        ...
        for result in results:
            if not result["success"]:
                print(f"  • {result['language']} at {result['rate_label']}: {result['error']}")
        return 1
